...
//  }
  ...
  def prepareEntity(multipartIdentifier: Seq[String]): SACAtlasReferenceable = {
    val tableIdentifier = multipartIdentifier match {
      case Seq(part1) => TableIdentifier(part1)
      case Seq(part1, part2) => TableIdentifier(part1, Some(part2))
      case _ =>
        throw new UnsupportedOperationException("Unsupport identifier: " + multipartIdentifier)
    }
    prepareEntity(tableIdentifier)
  }
