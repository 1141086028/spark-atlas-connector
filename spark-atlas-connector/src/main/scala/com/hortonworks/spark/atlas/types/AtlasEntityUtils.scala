...
import com.hortonworks.spark.atlas.utils.SparkUtils.{hiveConf, isHiveEnabled}
import org.apache.spark.sql.catalyst.catalog.{CatalogDatabase, CatalogStorageFormat, CatalogTable}
import com.hortonworks.spark.atlas.{AtlasClientConf, SACAtlasEntityWithDependencies,
...
trait AtlasEntityUtils extends Logging {
  ...
  def tableToEntity(
    ...
    val mockHiveConf: Option[Configuration] = None
    val conf = mockHiveConf.getOrElse(hiveConf)
    val usingRemoteMetastoreService = isHiveEnabled() && conf.getTrimmed("hive.metastore.uris",
      "").nonEmpty

    if (usingRemoteMetastoreService) {
