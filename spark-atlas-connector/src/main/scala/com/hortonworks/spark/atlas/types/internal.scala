...
import com.hortonworks.spark.atlas.{
  AtlasUtils, SACAtlasEntityWithDependencies,
...
//    val table = SparkUtils.getTableName(tableDefinition)
    ...
    // 待定用哪个
    //    val db = SparkUtils.sparkSession.sessionState.catalog.getCurrentDatabase
    val db: String = tableDefinition.database
