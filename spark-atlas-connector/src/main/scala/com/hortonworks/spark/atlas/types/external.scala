...
object external extends Logging {
  ...
  def hbaseTableToEntity(cluster: String, tableName: String, nameSpace: String): SACAtlasEntityWithDependencies = {
  ...
  def rdbmsTableToEntity(url: String, tableName: String): SACAtlasEntityWithDependencies = {
    ...
    jdbcEntity.setAttribute("qualifiedName", getRdbmsQualifiedName(getClusterName(), databaseName, tableName))
    jdbcEntity.setAttribute("name", tableName)
    SACAtlasEntityWithDependencies(jdbcEntity)
  }

  def hiveTableToReference(tblDefinition: CatalogTable, cluster: String, mockDbDefinition: Option[CatalogDatabase] = None): SACAtlasReferenceable = {
  ...
  private def toDbEntity(db: Database): AtlasEntity = {
    ...
    if (db.getOwnerType != null) ret.setAttribute(ATTRIBUTE_OWNER_TYPE, OWNER_TYPE_TO_ENUM_VALUE.get(db.getOwnerType.getValue))
    ret
  }
  ...
  private def toTableEntity(table: Table): Tuple2[AtlasEntity, Seq[SACAtlasReferenceable]] = {
    ...
    val lastAccessTime = if (table.getLastAccessTime > 0) table.getLastAccessTime * MILLIS_CONVERT_FACTOR
    else createTime
    ...
    if (table.getViewOriginalText != null) ret.setAttribute(ATTRIBUTE_VIEW_ORIGINAL_TEXT, table.getViewOriginalText)
    if (table.getViewExpandedText != null) ret.setAttribute(ATTRIBUTE_VIEW_EXPANDED_TEXT, table.getViewExpandedText)
  ...
  protected def getStorageDescEntity(tableId: AtlasObjectId, table: Table): AtlasEntity = {
    ...
    if (sd.getBucketCols != null && sd.getBucketCols.size > 0) ret.setAttribute(ATTRIBUTE_BUCKET_COLS, sd.getBucketCols)
  ...
  protected def getColumnEntities(tableId: AtlasObjectId, table: Table, fieldSchemas: util.List[FieldSchema]): util.List[AtlasEntity] = {
    ...
    if (CollectionUtils.isNotEmpty(fieldSchemas)) {
      ...
      for (fieldSchema <- fieldSchemas) {
        ...
        column.setAttribute(ATTRIBUTE_COL_POSITION, {
          columnPosition += 1; columnPosition - 1
        })
  ...
  protected def getQualifiedName(db: Database): String =  (db.getName + QNAME_SEP_CLUSTER_NAME).toLowerCase + getClusterName

  private def getQualifiedName(table: Table) = {
    var tableName = table.getTableName
    if (table.isTemporary) if (SessionState.get != null && SessionState.get.getSessionId != null) tableName = tableName + TEMP_TABLE_PREFIX + SessionState.get.getSessionId
    else tableName = tableName + TEMP_TABLE_PREFIX + RandomStringUtils.random(10)
    (table.getDbName + QNAME_SEP_ENTITY_NAME + tableName + QNAME_SEP_CLUSTER_NAME).toLowerCase + getClusterName
  }

  protected def getQualifiedName(table: Table, sd: StorageDescriptor): String = getQualifiedName(table) + "_storage"
  ...
  protected def getQualifiedName(table: Table, column: FieldSchema): String = {
    ...
    else tblQualifiedName.substring(0, sepPos) + QNAME_SEP_ENTITY_NAME + column.getName.toLowerCase + tblQualifiedName.substring(sepPos)
  }

  private def getHiveTable(databaseName: String, tableName: String): Table = Hive.get(SessionState.getSessionConf).getTable(databaseName, tableName)
  private def getHiveDatabase(databaseName: String): Database = Hive.get(SessionState.getSessionConf).getDatabase(databaseName)
  ...
  private def getTopicQualifiedName(clusterName: String, topic: String) = String.format(FORMAT_KAKFA_TOPIC_QUALIFIED_NAME, topic.toLowerCase, clusterName)
  ...
  private def toReferencedHBaseTable(cluster: String, tableName: String, nameSpace: String) = {
    ...
    if (hbaseTableName != null) {
      ...
      nsEntity.setAttribute(ATTRIBUTE_QUALIFIED_NAME, getHBaseNameSpaceQualifiedName(cluster, hbaseNameSpace))
      ...
      ret.setAttribute(ATTRIBUTE_QUALIFIED_NAME, getHBaseTableQualifiedName(cluster, hbaseNameSpace, hbaseTableName))
    }
    ret
  }

  private def getHBaseTableQualifiedName(clusterName: String, nameSpace: String, tableName: String) = String.format("%s:%s@%s", nameSpace.toLowerCase, tableName.toLowerCase, clusterName)

  private def getHBaseNameSpaceQualifiedName(clusterName: String, nameSpace: String) = String.format("%s@%s", nameSpace.toLowerCase, clusterName)
  ...
  private def getPathEntity(path: Path) = {
    ...
    if (isS3Path(strPath)) {
      ...
      val bucketQualifiedName = (path.toUri.getScheme + SCHEME_SEPARATOR + path.toUri.getAuthority + QNAME_SEP_CLUSTER_NAME).toLowerCase + getClusterName
      ...
      if (StringUtils.isNotEmpty(nameServiceID)) ret.setAttribute(ATTRIBUTE_NAMESERVICE_ID, nameServiceID)
  ...
  private def getQualifiedName(path: String): String = {
    if (path.startsWith(HdfsNameServiceResolver.HDFS_SCHEME)) return path + QNAME_SEP_CLUSTER_NAME + getClusterName
    path.toLowerCase
  }
  ...
  def getObjectId(entity: AtlasEntity): AtlasObjectId = {
    ...
    val ret = new AtlasObjectId(entity.getGuid, entity.getTypeName, Collections.singletonMap(ATTRIBUTE_QUALIFIED_NAME, qualifiedName).asInstanceOf[util.Map[String, AnyRef]])
    ret
  }
  ...
  private def isS3Path(strPath: String) = strPath != null && (strPath.startsWith(S3_SCHEME) || strPath.startsWith(S3A_SCHEME))
